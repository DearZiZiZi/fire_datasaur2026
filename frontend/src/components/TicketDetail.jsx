...
export default function TicketDetail({ ticketId, onClose }) {
    ...
    return (
        <div className="absolute top-0 right-0 bottom-0 w-[550px] bg-bg-primary border-l border-border z-50 shadow-2xl flex flex-col pt-[64px] animate-in slide-in-from-right duration-300 font-sans" style={{ top: '-64px' }}>
            ...
            {/* Drawer Header */}
            <div className="flex items-center justify-between px-6 bg-bg-secondary border-b border-border h-[64px] shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 bg-bg-secondary p-5 rounded-xl border border-border">
                    <div>
                ...
                {isDone && (
                    <div className="border border-brand-green/20 bg-brand-green/5 p-5 rounded-xl">
                        ...
                        <div className="flex flex-wrap gap-2 mb-5">
                            <span className="px-3 py-1 bg-bg-tertiary border border-brand-green/20 text-brand-green rounded-md text-xs font-semibold">{ticket.request_type}</span>
                            <span className="px-3 py-1 bg-bg-tertiary border border-brand-green/20 text-brand-green rounded-md text-xs font-semibold">{ticket.tone}</span>
                            <span className={`px-3 py-1 rounded-md border text-xs font-semibold ${ticket.priority_score >= 8 ? 'bg-accent-red/10 text-accent-red border-accent-red/20' : 'bg-bg-tertiary border-brand-green/20 text-brand-green'}`}>Priority: {ticket.priority_score}/10</span>
                        </div>

                        <div className="mb-5">
                            <div className="text-text-muted text-xs font-semibold uppercase mb-2 tracking-wider">Strategic Summary</div>
                            <p className="text-sm leading-relaxed text-text-secondary font-medium border-l-[3px] border-brand-green pl-3.5 bg-bg-tertiary/50 py-1">{ticket.ai_summary}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="bg-bg-tertiary border border-border rounded-lg p-4 text-sm font-medium text-text-secondary italic leading-relaxed whitespace-pre-line">
                    ...
                    <div className="fb-card p-5">
                        ...
                        <div className="flex items-center justify-between bg-bg-primary p-4 rounded-lg border border-border">
                            <div className="flex flex-col items-center flex-1 text-center">
                                <div className="p-2.5 bg-bg-secondary rounded-full border border-border mb-2">
                        ...
                        <MapContainer center={[ticket.lat, ticket.lng]} zoom={5} scrollWheelZoom={false} className="w-full h-full" style={{ zIndex: 0 }}>
                            ...
                            {officeData?.lat && officeData?.lng && (
                                <>
                                    ...
                                    <Polyline positions={[[ticket.lat, ticket.lng], [officeData.lat, officeData.lng]]} color="#22C55E" weight={2} dashArray="5, 5" opacity={0.8} />
                                </>
                            )}
                        </MapContainer>
                    </div>
                ...
                <div className="pb-6">
                    ...
                    <div className="bg-bg-secondary border border-border rounded-xl p-5 text-sm font-medium text-text-secondary leading-relaxed">
                        {ticket.description || <span className="text-text-muted italic">No Description Provided</span>}
                    </div>
                    {ticket.attachments && (
                        <div className="mt-3 text-sm font-medium bg-bg-secondary rounded-lg border border-border p-3 flex items-center justify-between">
